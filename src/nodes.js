...
export const ProductNode = imageArgs =>
  createNodeFactory(PRODUCT, async node => {
    ...
    if (node.reviewsConnection) {
      const reviewsConnection = node.reviewsConnection.edges.map(
        edge => edge.node
      )

      node.reviewsConnection___NODE = reviewsConnection.map(review =>
        generateNodeId(PRODUCT_REVIEWS_CONNECTION, review.id)
      )
      delete node.reviewsConnection
    }
...
export const ProductReviewsConnectionNode = _imageArgs =>
  createNodeFactory(PRODUCT_REVIEWS_CONNECTION)
